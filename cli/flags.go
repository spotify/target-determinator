...
func RegisterCommonFlags() *CommonFlags {
	...
	flag.Var(commonFlags.OpaqueInputRepositories, "opaque-input-repository", "Treat targets in this external repository as opaque inputs: hash their canonical labels and configurations, but not their contents or transitive dependencies. Use when repository contents are selected by invocation context, such as a repository rule materializing a comparison baseline, and content changes should not by themselves mark consumers as changed. This affects target selection only; Bazel execution still uses the real repository contents. Accepts an apparent or canonical repository name without leading @ characters. Repeatable.")
