...
`--opaque-input-repository=<name>` excludes the contents and transitive dependencies of an external repository from recursive target hashes while preserving each dependency's canonical label and configuration. The flag is repeatable and accepts apparent or canonical repository names without leading `@` characters. This is useful for repositories whose contents intentionally vary with comparison context rather than with the checked-out revision. It affects target selection only; Bazel still hashes the real files when executing a selected target. Callers should ensure that configuration which controls an opaque repository remains a normal, non-opaque input of the relevant validation targets.
