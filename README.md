...
`--opaque-input-repository=<name>` excludes the contents and transitive dependencies of an external repository from recursive target hashes while preserving each dependency's canonical label and configuration. The flag is repeatable and accepts apparent or canonical repository names without leading `@` characters.

For example, a repository rule may materialize files from a baseline revision selected by the CI invocation. The same checked-out source commit can then see different external repository contents when it is used in different comparisons. If persisted target hashes are keyed by source commit, recursively hashing those contents makes the hash context-dependent: an unchanged consumer can appear affected only because the selected baseline moved. Marking that repository opaque records that the consumer depends on the snapshot without treating the snapshot's comparison-specific contents as changes to the consumer.

This affects target selection only. If the consumer is selected for another reason, Bazel still uses and hashes the real repository contents when executing it. Callers must ensure that changes which should select the consumer—such as its current source inputs or comparison policy—remain normal, non-opaque inputs.
